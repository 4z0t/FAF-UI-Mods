...
TeamViewsContainer = Class(ArmyViewsContainer)
{
    ...
    _InitArmyViews = function(self)
        ...
        local teamIds = armiesData | LuaQ.select(function(armyData) return armyData.teamId end) | LuaQ.toSet


        local teams = teamIds |
            LuaQ.select.keyvalue(function(id)
                return armiesData | LuaQ.where(function(armyData)
                    return armyData.teamId == id
                end)
            end)
        reprsl(teams)
