...
function Main(isReplay)
    ...
    ---@return number?
    ---@return T?
    local function ReverseIPairsIterator(t, i)
        if i <= 1 then
            return nil, nil
        end
        i = i - 1
    ...
    ---@generic T
    ---@param t T[]
    ---@return fun(t:T[], i:number):(number?, T?)
    ---@return T[]
    ---@return number
    local function ReverseIPairs(t)
        return ReverseIPairsIterator, t, TableGetN(t) + 1
    end
    ...
    ---@generic K,V,I
    ---@param it fun(key:K):I,V
    ---@param k K
    ---@return I
    ---@return V
    local function CallIter(it, k)
        return it(k)
    end
    ...
    ---@param comparer? fun(left:V, right:V):boolean
    ---@return Enumerator
    function EnumeratorMeta:Sort(comparer)
        return EnumeratorCreate(SortTransformer(self.iterator, self.transformer, comparer))
    end
