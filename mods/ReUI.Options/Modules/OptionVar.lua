...
---@class OptionVar<T> : { Set:(fun(self:OptionVar<T>,value:T)), Bind:(fun(self:OptionVar<T>, f:fun(opt:Opt<T>))), Raw:(fun(self:OptionVar<T>):Lazy<T>) }
...
function Create(modOptionName, subOption, default)
    modOptionName = FormatName(modOptionName)
    subOption = FormatName(subOption)
